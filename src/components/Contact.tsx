  ...
  return (
    <section id="contact" className="py-32 px-6 bg-card">
      <div className="max-w-2xl mx-auto text-center">
        <span className="text-sm uppercase tracking-widest text-muted-foreground mb-4 block">Присоединиться</span>
        <h2 className="font-serif text-4xl md:text-5xl tracking-tight text-foreground mb-6 text-balance">
          Откройте мир традиций
        </h2>
        <p className="text-muted-foreground text-lg mb-10 leading-relaxed">
          Подпишитесь — и каждую неделю получайте интересный обычай или суеверие из России или Британии прямо на почту.
        </p>

        {submitted ? (
          <div className="p-8 rounded-2xl bg-sage/10 border border-sage/20">
            <p className="text-foreground font-serif text-2xl mb-2">Добро пожаловать!</p>
            <p className="text-muted-foreground">Проверьте почту — мы уже отправили первый материал.</p>
          </div>
        ) : (
